...
SCRIPT_PATH="./scripts/check_bpf_mount.sh"
if [ -f "$SCRIPT_PATH" ]; then
    $SCRIPT_PATH
else
    echo -e "${YELLOW}Warning: Script $SCRIPT_PATH not found. Skipping BPF mount check.${NC}"
fi
...
SCRIPT_PATH="./scripts/check_kernel_compat.sh"
if [ -f "$SCRIPT_PATH" ]; then
    $SCRIPT_PATH
else
    echo -e "${YELLOW}Warning: Script $SCRIPT_PATH not found. Skipping kernel compatibility check.${NC}"
fi

if [ $? -ne 0 ]; then
    echo -e "${YELLOW}Warning: Your kernel may not fully support all eBPF features.${NC}"
    echo "You may need to update your kernel to use this software properly."
fi

# Step 4: Check BTF information
echo -e "\n${BLUE}Step 4: Checking BTF support...${NC}"
SCRIPT_PATH="./scripts/check_btf.sh"
if [ -f "$SCRIPT_PATH" ]; then
    $SCRIPT_PATH
else
    echo -e "${YELLOW}Warning: Script $SCRIPT_PATH not found. Skipping BTF support check.${NC}"
fi
if [ $? -ne 0 ]; then
    echo -e "${YELLOW}Warning: BTF support is not available. This may affect the functionality of the firewall.${NC}"
fi
...
if [ $? -ne 0 ]; then
    ...
    # Verify the scripts were created and are executable
    if [ -f "build/load_firewall.sh" ] && [ -x "build/load_firewall.sh" ]; then
        echo -e "${GREEN}Successfully created load_firewall.sh${NC}"
    else
        echo -e "${YELLOW}Warning: load_firewall.sh was not created or is not executable${NC}"
    fi
fi
